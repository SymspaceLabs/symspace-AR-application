...
using static UnityEngine.XR.Interaction.Toolkit.Transformers.ARTransformer;
...
//{
    ...
    public class ARInteractorSpawnTrigger : MonoBehaviour
    {
        ...
        bool IsValidPlaneForTranslation(PlaneAlignment alignment, PlaneClassifications classification)
        {
            // Check if we have a valid object selected
            if (m_ObjectSpawner.objectIndex == -1) return false;

            // Get transformer script from the object
            ARTransformer transformer = m_ObjectSpawner.objectPrefabs[m_ObjectSpawner.objectIndex]
                                        .GetComponent<ARTransformer>();

            // First, check alignment (horizontal/vertical)
            bool validAlignment = false;
            switch (transformer.objectPlaneTranslationMode)
            {
                case PlaneTranslationMode.Horizontal:
                    validAlignment = alignment == PlaneAlignment.HorizontalUp;
                    break;
                case PlaneTranslationMode.Vertical:
                    validAlignment = alignment == PlaneAlignment.Vertical;
                    break;
                case PlaneTranslationMode.Any:
                    validAlignment = alignment == PlaneAlignment.HorizontalUp || alignment == PlaneAlignment.Vertical;
                    break;
                default:
            ...
            if (!validAlignment) return false;

            //// Now check classification (e.g., Floor, Table)
            //if (transformer.planeClassificationsList == null || transformer.planeClassificationsList.Count == 0)
            //    return true; // No restriction on classification
            Debug.Log("object classication : " + transformer.planeClassifications + "\nplane Classification : " + classification);
#if Unity_IOS
            return transformer.planeClassifications.Equals(classification);/* Contains(classification);*/
#else
            return true;
#endif
        }
