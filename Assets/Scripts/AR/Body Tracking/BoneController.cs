...
public class BoneController : MonoBehaviour
{
    ...
    public void ApplyObjectPosition(Transform item, ARHumanBody body, JointIndices jointIndex, Vector3 localOffset)
    {
        var joints = body.joints;
        if (!joints.IsCreated)
            return;

        item.transform.localPosition = joints[(int)jointIndex].localPose.position + localOffset;
    }

    public void ProcessJoint(Transform joint)
    {
    ...
    public Transform GetJointTransform(JointIndices jointIndex)
    {
        return m_BoneMapping[(int)jointIndex];
    }
